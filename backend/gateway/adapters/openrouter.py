"""
OpenRouter Provider Adapter — cloud aggregator with OpenAI-compatible API.
"""
...
# Cost per million tokens for common OpenRouter models
_MODEL_COSTS: dict[str, tuple[float, float]] = {
    "moonshotai/kimi-k2": (0.60, 0.60),
    "moonshotai/kimi-k2:thinking": (0.60, 2.00),
...
class OpenRouterAdapter(BaseProviderAdapter):
    ...
    def __init__(self) -> None:
        secret = get_provider_key("openrouter")
    ...
    def _headers(self) -> dict[str, str]:
        return {
            ...
            "HTTP-Referer": "https://agentop.local",
            "X-Title": "Agentop Gateway",
        }
    ...
    async def chat_complete(self, request: ChatCompletionRequest) -> ChatCompletionResponse:
        ...
        return ChatCompletionResponse(
            id=data.get("id", f"or-{uuid.uuid4().hex[:12]}"),
            model=request.model,
    ...
    async def chat_stream(self, request: ChatCompletionRequest) -> AsyncIterator[StreamChunk]:
        ...
        body = self._build_body(request)
        body["stream"] = True
        comp_id = f"or-{uuid.uuid4().hex[:12]}"
        async with httpx.AsyncClient(timeout=120) as client:
            try:
