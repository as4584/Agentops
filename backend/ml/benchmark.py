...
class BenchmarkSuite:
    ...
    def get_baseline(self, suite_id: str, model: str) -> SuiteReport | None:
        key = f"{suite_id}__{model}"
        path = self._baselines_dir / f"{key}.json"
        if not path.exists():
            return None
    ...
    def _detect_regressions(
        ...
        if current.pass_rate < baseline.pass_rate - 0.05:
            regressions.append(f"Pass rate dropped: {baseline.pass_rate:.2f} → {current.pass_rate:.2f}")

        # Average score regression
        if current.avg_score < baseline.avg_score - 0.05:
            regressions.append(f"Avg score dropped: {baseline.avg_score:.2f} → {current.avg_score:.2f}")

        # Latency regression (>20% increase)
        if baseline.avg_latency_ms > 0 and current.avg_latency_ms > baseline.avg_latency_ms * 1.2:
            regressions.append(f"Latency increased: {baseline.avg_latency_ms:.0f}ms → {current.avg_latency_ms:.0f}ms")
    ...
    @staticmethod
    def create_webgen_suite(golden_sites_dir: str | None = None) -> dict[str, Any]:
