...
class RetrievalEngine:
    ...
    async def retrieve(
        ...
        if corpus_filter and dense_hits:
            dense_hits = [
                ...
                if any(str(h.get("path", "")).startswith(p) for p in corpus_filter)
            ]
        ...
        if self.bm25.available:
            try:
                ...
                logger.warning(
                    f"[RetrievalEngine] BM25 search failed: {exc} "
    ...
    def _reciprocal_rank_fusion(
        ...
        for rank, hit in enumerate(dense):
            cid = (
                hit.get("chunk_id")
                or f"{hit.get('path', '')}::{hit.get('chunk_index', rank)}"
            )
    ...
    def retrieval_health(self) -> dict[str, Any]:
        ...
        return {
            ...
            "active_method": (
                "hybrid_rrf" if self.bm25.available else "dense_only"
            ),
        }
