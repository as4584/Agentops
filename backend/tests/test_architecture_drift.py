...
class TestArchitectureDriftInvariants:
    ...
    def test_drift_detected_when_unapproved_agent_has_gitnexus(self):
        ...
        with patch.object(
            vad,
            "ALL_AGENT_DEFINITIONS" if hasattr(vad, "ALL_AGENT_DEFINITIONS") else "__builtins__",
            fake_agent_defs,
            create=True,
        ):
            ...
            if rogue_gn and "rogue_agent" not in vad._APPROVED_GITNEXUS_AGENTS:
                failures.append("DRIFT: Agent 'rogue_agent' has GitNexus permissions but is not in approved list.")
